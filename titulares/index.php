...
$resultado = buscarTitulares($busqueda, $pagina, $limite);
$titulares = $resultado['datos'];
$total = $resultado['total'];
$total_paginas = ceil($total / $limite);

$titulo = 'Lista de Titulares';
include '../includes/header.php';
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="fas fa-building"></i> Titulares</h2>
    <a href="<?php echo BASE_URL; ?>titulares/crear.php" class="btn btn-primary"><i class="fas fa-plus"></i> Nuevo Titular</a>
</div>
                ...
                <tbody>
                    <?php if (count($titulares) > 0): ?>
                        <?php foreach ($titulares as $t): ?>
                            <tr>
                                <td><?php echo $t['id_titular']; ?></td>
                                <td><strong><?php echo h($t['nombre']); ?></strong></td>
                                <td><?php echo h($t['pais_nombre']); ?></td>
                                <td><?php echo h($t['direccion_texto']); ?></td>
                                <td><?php echo h($t['telefono']); ?></td>
                                <td><?php echo h($t['correo']); ?></td>
                                <td>
                                    <a href="<?php echo BASE_URL; ?>titulares/editar.php?id_titu=<?php echo $t['id_titular']; ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                    <a href="<?php echo BASE_URL; ?>titulares/eliminar.php?id_titu=<?php echo $t['id_titular']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="7" class="text-center">No hay titulares que coincidan con la búsqueda.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
